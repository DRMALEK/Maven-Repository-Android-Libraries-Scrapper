...
def extract_realeses_url(artifact_page_response, artifact_url, all_versions):
    ...
    # If not all releases is choosen, just return the last release
        ...
        versions_table_first_row = versions_table.find(name="a", attrs={"class": "vbtn release"})
            ...
            with open(os.path.join("out_repo" , artifact_name, "info.json"), "w+") as info_file:
                ...
                for url_element in content.find_all(name="a"):
                    ...
                    try:
                        ...
                        with open(os.path.join("out_repo" , 
                                              artifact_name , 
                                              artifact_version , 
                                              url_href), "wb") as output:
    ...
    for request_param in request_params:
        ...
        if current_page :
            try:
                ...
                # This excpetion means that there is a problem in parsing
                if str(excp) == "'NoneType' object is not subscriptable": 
                    continue
...
if __name__ == "__main__":
    try:
        ...
        print("Usage: python3 mvn_scrapper.py [repo name] \
            [sorting criteria] \
            [sleep duration] \
            [maximum tries per url] \
            [all versions]")
        exit
    ...
    if sorting_criteria not in sorting_criterias and sorting_criteria != "all":
        print("Error please choose one of the supported criterias ! : \
              " + " ".join(i for i in sorting_criterias) + " all")
        exit(1)
    ...
    try:
        ...
        print("Error a directory named out_repo exists ! \
              please delete it or move it to another directory and try again")
        exit(1)
